  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://data-api-d6lk.onrender.com/login",
        login
      );
      setisloading(false);
      ...
      <div className="bg-white h-4/5 w-full sm:w-1/2 rounded-md flex shadow-xl overflow-hidden justify-center">
        ...
        <div className="bg-[#fb7660] w-1/2 h-full flex flex-col justify-center rounded-r-md p-4 sm:p-8">
          <form onSubmit={handleSubmit} className="flex flex-col items-center">
            <h1 className="font-bold text-lg mb-8 text-[#174660] text-center">
              Login
            </h1>
            <input
            ...
            />
            <button
              className="bg-[#174660] px-4 py-1 text-white rounded-xl mt-5 focus shadow-md cursor-pointer"
              type="submit"
            >
              Login
            </button>
            {isloading && !loginError && (
              <div className="h-8 w-8 animate-spin rounded-full border-4 border-solid border-current border-r-transparent text-secondary align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite] mt-4"></div>
            )}
            {loginError && (
